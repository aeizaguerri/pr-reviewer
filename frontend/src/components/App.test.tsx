...
describe("App smoke UI", () => {
  ...
  it("renders the app shell and loaded providers without exposing a review form", async () => {
    ...
    expect(screen.getByRole("heading", { name: /pr code reviewer/i })).toBeInTheDocument();
    ...
    expect(screen.getByText(/cerebras/i)).toBeInTheDocument();
  ...
  it("renders sanitized API errors when provider loading fails", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("token ghp_secret should not leak")));
